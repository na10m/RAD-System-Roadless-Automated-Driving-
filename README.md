...
```
# Phase of Work Description Key Deliverables Key Dates
1 Concept Research
and Parts
Procurement
```
- Create a list of hardware,
    ...
    else that needs to be sourced
    for the project
       - Spreadsheet of components needed +
          pricing, where components will be
          purchased from
       - Find, order, purchase all components
          - Spreadsheet completed by
             **October 10th, 2021**
          - All parts ordered/purchased
             by **October 13th, 2021**
2 Sensor Suite
Assembly and ML
Model Training
- Set up all sensors (camera,
LiDAR, etc.) and test
connectivity with
microcontroller
- Develop approach for ML
model; determine which
third-party tools, libraries, and
test data sets should be used
based on hardware
constraints and software
compatibility
- Tests that show
i. Correct readings from sensors (e.g.
video frames from camera)
ii. Proof of connectivity with
microcontroller
- Documented plan of machine learning
approach including:
i. list of software tools to be used
ii. high-level outline detailing how ML
model with be trained and integrated
with the system
- Tests and ML plan
completed by **October 22nd,
2021**

```
3 Road Classification^ • Perform image recognition,
environment model
construction, and make
preliminary decisions for
navigations (e.g., pathway
classification) on
microcontroller
```
- Image recognition test showing correct
    identification of objects using the ML model
- Environment model test showing data from
    all hardware sensors being used to identify
    waypoint, track width, and pathway surface
       - Image recognition test
          completed by **November**
          **21 st, 2021**
       - Environment modelling test
          completed by **December**
          **26 th, 2021**
4 Navigation^ • Use road classification model
to determine navigation
parameters
- Test demonstrating road classification
model being used to plan vehicle
orientation, velocity, and suspension
- Navigation test completed
by **January 23rd, 2021**


- Customize RC car and hijack
    remote control for use with
    computers
       - Driving test demonstrating custom control
          for RC car using a game controller or PC
          program
             - Driving test completed by
                **February 6th, 2021**

```
5 Vehicle Control^ • Develop system for translating
navigation data into control
signals to operate the vehicle
```
- Test demonstrating vehicle orientation and
    velocity control
       - Vehicle control test
          completed by **February 13th,**
          **2021**
6 Weather detection^ • Expand ML model to detect
non-ideal weather conditions
- Test involving simulation of various
inclement weather conditions to see if the
ML model can accurately detect them
- Weather detection test
completed by **February 27th,
2021**
7 Complete
Autonomous
Driving
...
off-road and non-ideal
weather conditions
- User interface for live
navigation data display
- Full autonomous driving test • Autonomous driving test
completed by **March 27th,
2021**

### Risks and Alternative Plan

With autonomous driving, there are three main tasks to achieve. To make our system see, to think, and to act.
Each of these design objectives comes with a risk that we must account for.

**Risk 1: Machine Learning Model does not perform at a high enough level; our system can’t see**

Solution: We instead of becoming completely autonomous we will make the car respond to various road
conditions to optimize for driving in them. These road conditions can be icy, muddy, or wet – the car can adjust
torque and suspension in real time to accommodate for these various road conditions.

**Risk 2: Machine Learning can’t be used to make the system make smart decisions; our system can’t think**

Our system will be used for object classification built with a user interface to assist in driving. This can be
implemented in any car.

**Risk 3 : Machine Learning Model does not integrate into the car correctly; our system can’t act**

Solution: We can test the model and see what the best lines are a driver can make in off road conditions. A user
interface can be made and could be used as a training system for driving.
...
```
- Perception software
    capable of classifying road
    trajectory and obstacles
- Vehicle Velocity
    Suggestion, based on
    environmental changes
       - Detection of non-ideal
          weather conditions
       - Partial Speed Control
          - Complete Machine
             Learning model
          - User Interface
